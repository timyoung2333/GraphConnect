function [res1, res2, objective] = vl_MLCOSLinear_fb(net, x1, x2, pLabels)
...
objective = -pLabels*cos_ij';

dzdy1 = bsxfun(@times,-pLabels,squeeze(res2(end).x));
dzdy1 = permute(dzdy1,[3 4 1 2]);
dzdy2 = bsxfun(@times,-pLabels,squeeze(res1(end).x));
