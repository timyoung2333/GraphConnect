function [net,info] = MLtrain_geoReg(X,trainLabel,net,paras,doVal)
...
%---------------------------------------------------------------
 ...
 pairs = pairs(:,randperm(pairSiz));
  ...
  for b=1:paras.B+1
    ...
    [res1, res2, objective_thisBatch, dtau] = vl_MLfb_geoReg(net, im1, im2, paras.lambda, tau, pLabels, full(G(sub2ind([N N],i,j))));
    
    % gradient step
    tau = tau-dtau*0;%paras.eta;
  ...
  info.tau(end+1) = tau;
  clear res1 res2;
  fprintf('  trainLoss %.4e,', info.trainObj(end));
  ...
  if rem(e-E0,paras.regen)==0 % regenerate the pairs
  ...
  if rem(e-E0,paras.retau)==0 && rem(e-E0,paras.regen)~=0  % recompute tau
