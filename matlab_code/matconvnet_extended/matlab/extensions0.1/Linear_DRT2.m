function [A, info] = Linear_DRT2(X,trainLabels,paras)
...
function [cost, loss, grad, intraDists, interDists] = trainIt(A)
    ...
    grad_discrimination = 1/length(pairLabels) * ( (Y(:,effPos(1,:))-Y(:,effPos(2,:))) * (X(:,effPos(1,:))-X(:,effPos(2,:)))'...
                            -(Y(:,effNeg(1,:))-Y(:,effNeg(2,:))) * (X(:,effNeg(1,:))-X(:,effNeg(2,:)))');
                            
    grad_robustness = 1/length(neighborPairsID)* ( bsxfun(@times, sign(distdat(neighborPairsID)-full(D(neighborPairsID))), Y(:,neighborPairs(1,:))-Y(:,neighborPairs(2,:)) )...
