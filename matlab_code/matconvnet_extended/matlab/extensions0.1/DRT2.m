function [net,info] = DRT2(X,trainLabel,net,paras)
...
%---------------------------------------------------------------
 ...
 regu = regu(permInd);
  ...
  for b=1:paras.B+1
    ...
    [res1, res2, dtau] = vl_DRT2fb(net, X(:,:,:,pairBatch(1,:)), X(:,:,:,pairBatch(2,:)), tau, pL, r, paras.lambda);
...
function regu = computeRegularizer(X,pairs,trainLabel)
    ...
    if paras.kn==inf
        ...
        %regu(pairLabels==1) = regu(pairLabels==1)/mean(regu(pairLabels==1));
        ...
        %regu(inNeighbor) = regu(inNeighbor)/mean(regu(inNeighbor));
    end
end
...
function [loss, cost, intraDists, interDists] = computeTrainLoss(X)
    ...
    loss = mean(max(-pairLabels.*(tau-dists), 0));
    cost = loss+paras.lambda/length(pairLabels)*sum(abs( dists(regu~=0)-full(regu(regu~=0)) ));
...
function [loss, intraDists, interDists] = computeTestLoss(X)
    gramX = X'*X;
    normX = sum(X.^2,1);
    distsX = bsxfun(@plus,bsxfun(@plus,normX,-2*gramX),normX');
    distsX(1:N_test+1:end) = 0;
    distsX = squareform(gather(distsX),'tovector');
    loss = mean( max(-testpairLabels.*(tau-distsX),0) );
