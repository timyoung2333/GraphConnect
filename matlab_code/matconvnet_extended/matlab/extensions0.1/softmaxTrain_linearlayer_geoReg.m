function [net, info, G] = softmaxTrain_linearlayer_geoReg(trainX,trainLabel,net,paras)
...
if ~isfield(paras,'G')
    G = cell(1,length(net.layers));
    G{1} = learnGraph(trainX,trainLabel);
    for i=2:length(net.layers)
        if (strcmp(net.layers{i}.type,'conv')||strcmp(net.layers{i}.type,'innerproduct')) && ~strcmp(net.layers{i+1}.type,'softmaxloss')
            G{i} = G{1};
        end
    end
else 
...
for e=E0+1:E0+paras.E
  ...
  % do a forward pass for computing graphs
%   collect_layer_outputs = cell(1,length(net.layers));
%   for t=1:paras.B:trainSiz
%     batch = t:min(t+paras.B-1, trainSiz) ;
%     net.layers{end}.class = trainLabel(batch) ;
%     res = vl_simplenn_extended(net,trainX(:,:,:,batch));
%     for i=2:length(res)-1
%         if (strcmp(net.layers{i}.type,'conv')||strcmp(net.layers{i}.type,'innerproduct')) && ~strcmp(net.layers{i+1}.type,'softmaxloss')
%             collect_layer_outputs{i} = cat(4, collect_layer_outputs{i}, res(i).x);
%         end
%     end
%   end
%   for i=2:length(res)-1
%         if ~isempty(collect_layer_outputs{i})
%             G{i} = learnGraph(collect_layer_outputs{i},trainLabel');
%         end
%   end
  ...
  for t=1:paras.B:trainSiz
    ...
    for i=1:length(G)
        if ~isempty(G{i})
            L{i} = full(diag(sum(G{i}(batch,batch),2))-G{i}(batch,batch));
        end
    end
    res = vl_simplenn_extended_linearlayer_geoReg2(net, trainX(:,:,:,batch), one, [], L, paras.lambda) ;
