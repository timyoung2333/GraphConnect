function res = vl_simplenn_extended_linearlayer_geoReg(net, x, dzdy, res, L, lambda, varargin)
...
if doder
  ...
  for i=n:-1:max(1, n-opts.backPropDepth+1)
    ...
    if i>1 && lambda(i-1)~=0 && ~isempty(L{i-1})% manifold regularizer
        res(i).dzdx = res(i).dzdx+reshape(lambda(i-1)*reshape(res(i).x,[],size(res(i).x,4))*L{i-1},size(res(i).x));
    end
