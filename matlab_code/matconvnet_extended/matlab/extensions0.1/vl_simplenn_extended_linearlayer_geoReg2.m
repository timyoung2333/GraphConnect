function res = vl_simplenn_extended_linearlayer_geoReg2(net, x, dzdy, res, L, lambda, varargin)
...
opts.onlyGradient = false; % added: only compute layer-wise gradient, forward pass result and top layer gradient must be given
...
if (~exist('res','var')||isempty(res)||~exist('dzdy','var')||isempty(dzdy)) && opts.onlyGradient
    error('must provide layer-wise output and top-layer gradient!');
end
...
for i=1:n
  ...
  switch l.type
    ...
    case 'innerproduct' % added
        if isfield(l, 'weights')
            res(i+1).x = vl_nninnerproduct(res(i).x, l.weights{1}, l.weights{2});
        else
            res(i+1).x = vl_nninnerproduct(res(i).x, l.filters, l.biases);
        end
    case 'pool'
    ...
    case 'channelnormalize' % added
      res(i+1).x = vl_nnchannelnormalize(res(i).x);
    case 'softmax'
...
if doder
  ...
  for i=n:-1:max(1, n-opts.backPropDepth+1)
    ...
    switch l.type
          ...
          if isfield(l, 'weights')
            [res(i).dzdx, res(i).dzdw{1}, res(i).dzdw{2}] = ...
                vl_nnconv(res(i).x, l.weights{1}, l.weights{2}, ...
                          ...
                          'pad', l.pad, 'stride', l.stride, ...
                          cudnn{:}) ;
            if ~isempty(local_dzdx)
                [~,local_dzdw{1},local_dzdw{2}] = vl_nnconv(res(i).x, l.weights{1}, l.weights{2}, ...
                          local_dzdx, ...
                          'pad', l.pad, 'stride', l.stride, ...
                          cudnn{:}) ;
                res(i).dzdw{1} = res(i).dzdw{1}+local_dzdw{1};
                res(i).dzdw{2} = res(i).dzdw{2}+local_dzdw{2};
            end
      ...
      case 'innerproduct' % added
          if isfield(l, 'innerproduct')
              [res(i).dzdx, res(i).dzdw{1}, res(i).dzdw{2}] = ...
                vl_nninnerproduct(res(i).x, l.weights{1}, l.weights{2}, ...
                          res(i+1).dzdx);
          else
              [res(i).dzdx, res(i).dzdw{1}, res(i).dzdw{2}] = ...
                vl_nninnerproduct(res(i).x, l.filters, l.biases, ...
                          res(i+1).dzdx);
          end
      case 'pool'
      ...
      case 'channelnormalize' %added
        res(i).dzdx = vl_nnchannelnormalize(res(i).x,res(i+1).dzdx);
      case 'softmax'
    ...
    if i>1 && lambda(i-1)~=0 && ~isempty(L{i-1})% manifold regularizer
        local_dzdx = reshape(lambda(i-1)*reshape(res(i).x,[],size(res(i).x,4))*L{i-1},size(res(i).x));
    else
        local_dzdx = [];
    end
