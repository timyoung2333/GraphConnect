function [net, info] = softmaxTrain(trainX,trainLabel,net,paras)
...
for e=E0+1:E0+paras.E
  ...
  for t=1:paras.B:trainSiz
    ...
    res = vl_simplenn_extended(net, trainX(:,:,:,batch), one) ;
  ...
  if doTEST && rem(e-E0,paras.teststep)==0
      ...
      for testB = 1:paras.B:testSiz
          ...
          res_test = vl_simplenn_extended(net,paras.testdat(:,:,:,id),[],[], 'conserveMemory', false, 'sync', true,'disableDropout',true);
