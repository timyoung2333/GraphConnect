function res = vl_simplenn_extended_geoReg(net, x, dzdy, res, L, lambda, varargin)
...
if doder
  ...
  for i=n:-1:max(1, n-opts.backPropDepth+1)
    ...
    if i>1 && lambda(i-1)~=0% manifold regularizer
        res(i).dzdx = res(i).dzdx+reshape(lambda(i-1)*reshape(res(i).x,[],size(res(i).x,4))*L,size(res(i).x));
    end
