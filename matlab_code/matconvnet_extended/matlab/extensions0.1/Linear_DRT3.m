function [A, info] = Linear_DRT3(X,trainLabels,paras)
...
function mtx_normed = normalizeCol(mtx)
    mtx_normed = bsxfun(@rdivide,mtx,sqrt(sum(mtx.^2,1)));
end
...
function [cost, loss, grad, intraDists, interDists] = trainIt(A)
    ...
    effPos = find(pairLabels==1 & distdat>tau(1));
    effNeg = find(pairLabels==-1 & distdat<tau(2));
    grad{1} = [-numel(effPos), numel(effNeg)]/length(pairLabels); % gradient w.r.t. tau
    
    loss = (sum(distdat(effPos)-tau(1))+sum(tau(2)-distdat(effNeg)))/length(pairLabels);
    cost = loss+paras.lambda*sum(abs(distdat(neighborPairsID)-full(D(neighborPairsID))))/length(neighborPairsID) + paras.w*norm(A,'fro')^2;
    
    effPos_sub = indexMapping(trainSiz,effPos);
    effNeg_sub = indexMapping(trainSiz,effNeg);
    
    grad_discrimination = 1/length(pairLabels) * ( normalizeCol(Y(:,effPos_sub(1,:))-Y(:,effPos_sub(2,:))) * (X(:,effPos_sub(1,:))-X(:,effPos_sub(2,:)))'...
                            -normalizeCol(Y(:,effNeg_sub(1,:))-Y(:,effNeg_sub(2,:))) * (X(:,effNeg_sub(1,:))-X(:,effNeg_sub(2,:)))');
                            
    grad_robustness = 1/length(neighborPairsID)* ( bsxfun(@times, sign(distdat(neighborPairsID)-full(D(neighborPairsID))), normalizeCol(Y(:,neighborPairs(1,:))-Y(:,neighborPairs(2,:))) )...
                            * (X(:,neighborPairs(1,:))-X(:,neighborPairs(2,:)))'); 
                                             
    grad{2} = grad_discrimination+paras.lambda*grad_robustness+paras.w*A;
end
...
function [loss, testAcc, intraDists, interDists] = testIt(A)
    ...
    dists_test = sqrt(squareform(dists_test,'tovector'));
    ...
    loss = mean([max(0,dists_test(pairLabels==1)-tau(1)),max(0,tau(2)-dists_test(pairLabels==-1))]);   
