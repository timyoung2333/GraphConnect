function pairInfo = generatePairs4(labels,posRate,negRate,Xtrain)
% Generate pairs
% Input-
...
% Ouput-
% pairs: a 4-column array, pairs(k,1), pairs(k,2) are the k-th
%   pair's indices. pairs(k,3) is the label, +1/-1. pairs(k,4) is the
%   cosine similarity
...
if posRate==1 && negRate==1
    ...
    for k=unique(labels)
        ...
        if negRate~=0
            ...
            pairInfo = [pairInfo; [inds_neg, -ones(size(inds_neg,1),1), CS_ij'] ];
        end
    end
end
