    ...
    for epoch in range(num_epochs):
        ...
        for idx, (data, targets) in enumerate(trainloader):
            ...
            if mode == "one":
                ...
                print(f"    batch_train_loss: {loss:.4f}, original={loss:.4f}, J3={J1:.4f}", flush=True)
...
if __name__ == "__main__":
    ...
    log_file = open(f"{cwd}/log_eachclass/seed{seed}__num{num}_wd{wd}_lam{lam}_coef{coef_str}_J3_debug.log", 'w')
    sys.stdout = log_file
    ...
    filename = f"{cwd}/gc_mnist_result_eachclass/seed{seed}_num{num}_wd{wd}_lam{lam}_coef{coef_str}_J3_debug.csv"
